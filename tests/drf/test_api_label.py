...
TESTED_ENDPOINT = '/labels/'
pytestmark = pytest.mark.django_db


class TestLabelListAPI:
    full_endpoint = package_conftest.get_list_endpoint(TESTED_ENDPOINT)

    def test_api_label_get_success(
            self, api_client, label_factory, user_factory):
        user = user_factory()
        api_client.login(username=user.username, password=DEFAULT_PASSWORD)
        initial_data = label_factory.create_batch(3)
    ...
    def test_api_label_get_reject_anonymous_user(
            self, api_client):
        response = api_client.get(self.full_endpoint)
...
class TestLabelCreateAPI:
    full_endpoint = package_conftest.get_list_endpoint(TESTED_ENDPOINT)

    def test_api_label_post_success(
            self, api_client, label_factory, user_factory):
        user = user_factory()
        api_client.login(username=user.username, password=DEFAULT_PASSWORD)
        initial_data = label_factory.create_batch(3)
    ...
    def test_api_label_post_reject_existing_name(
    ...
    def test_api_label_post_reject_anonymous_user(self, api_client):
        expected_data = deepcopy(TEST_ITEMS[0])
...
class TestLabelRetriveAPI:
    def test_api_label_get(
            self, api_client, label_factory, user_factory):
        user = user_factory()
        api_client.login(username=user.username, password=DEFAULT_PASSWORD)
        initial_data = label_factory.create_batch(3)
    ...
    def test_api_label_get_reject_anonymous_user(
            self, api_client, label_factory):
        label_factory.create_batch(3)
        full_endpoint = package_conftest.get_last_item_endpoint(
            TESTED_ENDPOINT, PackageModel)
...
class TestLabelPutAPI:
    def test_api_label_put_success(
            self, api_client, label_factory, user_factory):
        user = user_factory()
        api_client.login(username=user.username, password=DEFAULT_PASSWORD)
        initial_data = label_factory.create_batch(3)
    ...
    def test_api_label_put_reject_anonymous_user(
            self, api_client, label_factory):
        label_factory.create_batch(3)
        full_endpoint = package_conftest.get_last_item_endpoint(
            TESTED_ENDPOINT, PackageModel)
...
class TestLabelDeleteAPI:
    def test_api_label_delete_success(
            self, api_client, label_factory, user_factory):
        user = user_factory()
        api_client.login(username=user.username, password=DEFAULT_PASSWORD)
        initial_data = label_factory.create_batch(3)
    ...
    def test_api_label_delete_reject_anonymous_user(
            self, api_client, label_factory):
        label_factory.create_batch(3)
        full_endpoint = package_conftest.get_last_item_endpoint(
            TESTED_ENDPOINT, PackageModel)
    ...
    def test_api_label_delete_reject_item_associated_with_task(
            self, api_client, label_factory, task_factory, user_factory):
        user = user_factory()
        api_client.login(username=user.username, password=DEFAULT_PASSWORD)
